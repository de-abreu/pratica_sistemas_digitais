...
def build_instrucoes_t_file(program_name, instructions, template):
    ...
    hexmap_vhdl_instructions = pipeline_compile_instructions_to_hexcode(
        instructions
    )  # mapear_opcode_hexcode(template):
    instructions_t_content = (
        template.replace("template_t", f"{program_name}_t")
        .replace("template", program_name)
        .replace("--hexcode_placeholder", hexmap_vhdl_instructions)
    )
...
instructions_t_file_txt = build_instrucoes_t_file(
    program_name, program_instructions, template_t
)
